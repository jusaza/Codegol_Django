...
class Rendimiento(models.Model):
    ...
    id_rendimiento = models.AutoField(
        primary_key=True
    )
        ...
        if self.valor is not None:
            ...
            if not existe_asistencia:
                raise ValidationError(
                    "No se puede registrar rendimiento sin asistencia"
                )
            ...
            if not actividad_valida:
                raise ValidationError(
                    "Actividad no válida para la posición"
                )
            ...
            if not atributo_valido:
                raise ValidationError(
                    "El atributo no corresponde a la actividad"
                )
